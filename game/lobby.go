...
type NameChangeEvent struct {
	PlayerID   string `json:"playerId"`
	PlayerName string `json:"playerName"`
    PlayerImage string `json:"playerImg"`
}
...
func handleNameChangeEvent(caller *Player, lobby *Lobby, name string) {
	oldName := caller.Name
    newTempData := strings.Split(name, ";")
	newName := SanitizeName(newTempData[0])
    newImg := newTempData[1]
    ...
    caller.Name = newName
    if(len(strings.TrimSpace(newImg)) <= 0 || newImg == "undefined") {
        newImg = caller.DisplayImage
    } else {
        newImg = strings.TrimSpace(newImg)
    }

    caller.DisplayImage = newImg
...
func CreateLobby(playerName, playerImage, chosenLanguage string, publicLobby bool, drawingTime, rounds, maxPlayers, customWordsChance, clientsPerIPLimit int, customWords []string, enableVotekick bool) (*Player, *Lobby, error) {
	lobby := &Lobby{
	...
	player := createPlayer(playerName, playerImage)
...
func (lobby *Lobby) JoinPlayer(playerName string) *Player {
	player := createPlayer(playerName, "/resources/character1.png")
